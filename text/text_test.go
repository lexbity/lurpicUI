...
func TestFontRegistry_new_no_error(t *testing.T) {
	...
	if got := reg.Resolve(TextStyle{}); got.IsZero() {
		t.Fatal("expected fallback face")
	}
}
...
func TestFontRegistry_load_font_bytes(t *testing.T) {
	reg, _ := NewFontRegistry()
	if err := reg.LoadFontBytes([]byte("fake-font-data"), "test"); err != nil {
		t.Fatalf("LoadFontBytes: %v", err)
	}
	if got := reg.Resolve(TextStyle{Family: "test"}); got.IsZero() {
...
func TestFontRegistry_resolve_respects_weight(t *testing.T) {
	reg, _ := NewFontRegistry()
	if err := reg.LoadFontBytes([]byte("regular"), "demo"); err != nil {
		t.Fatalf("LoadFontBytes regular: %v", err)
	}
	if err := reg.LoadFontBytes([]byte("bold"), "demo-bold"); err != nil {
		t.Fatalf("LoadFontBytes bold: %v", err)
	}
	regular := reg.Resolve(TextStyle{Family: "demo", Weight: WeightRegular})
	bold := reg.Resolve(TextStyle{Family: "demo-bold", Weight: WeightBold})
...
func TestLoadFontFile_roundtrip(t *testing.T) {
	...
	if got := reg.Resolve(TextStyle{Family: filepath.Base(path)}); got.IsZero() {
		t.Fatal("expected loaded face")
	}
}
...
func TestShaper_shape_simple_non_nil(t *testing.T) {
	...
	layout := shaper.ShapeSimple("abc", DefaultStyle())
	if layout == nil {
...
func TestShaper_wraps_max_width(t *testing.T) {
	reg, _ := NewFontRegistry()
	shaper := NewShaper(reg)
	layout := shaper.Shape(Paragraph{
		Spans:    []TextSpan{{Text: "abcdef", Style: DefaultStyle()}},
		MaxWidth: 15,
	})
	if layout.LineCount() < 2 {
		t.Fatalf("expected wrap, got %d lines", layout.LineCount())
	}
}
...
func TestShaper_alignment_center_and_right(t *testing.T) {
	...
	center := shaper.Shape(Paragraph{
		Spans:     []TextSpan{{Text: "abc", Style: DefaultStyle()}},
		MaxWidth:  100,
...
func TestTextLayout_empty_spans_valid(t *testing.T) {
	reg, _ := NewFontRegistry()
	shaper := NewShaper(reg)
	layout := shaper.Shape(Paragraph{Spans: []TextSpan{{Text: "", Style: DefaultStyle()}}})
	if layout == nil {
		t.Fatal("expected layout")
	}
...
func TestHitTest_right_half_of_glyph(t *testing.T) {
	...
	if pos.Index != first.Glyphs[0].RuneIndex+1 {
		t.Fatalf("pos = %#v", pos)
	}
}
...
func wrappedHelloWorld(t *testing.T) *TextLayout {
	...
	return shaper.Shape(Paragraph{
		Spans:     []TextSpan{{Text: "Hello World", Style: DefaultStyle()}},
		MaxWidth:  40,
		Alignment: AlignLeft,
	})
}
