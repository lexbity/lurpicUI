...
func AssertScrollRegionGolden(t *testing.T, name string, direction ScrollDirection, mutate func(*ScrollRegion), assert func(*ScrollRegion)) {
	...
	_ = sr.layoutRole.Measure(facet.MeasureContext{
		Runtime:          rt,
		Theme:            ctx,
		ContentScale:     1,
	...
	sr.layoutRole.Arrange(facet.ArrangeContext{
		Runtime:     rt,
		Theme:       ctx,
		ParentGroup: sr.layoutRole.Parent,
		ChildGroup:  sr.layoutRole.Child,
	}, canvas)
	if assert != nil {
		assert(sr)
	}
	cmds := sr.projectionRole.Project(facet.ProjectionContext{
		Runtime:      rt,
		Bounds:       canvas,
		ContentScale: 1,
	})
...
func scrollRegionVerticalChildren() []ScrollRegionChild {
	...
	for i := 0; i < 12; i++ {
		out = append(out, ScrollRegionChild{
			Facet:     primitive.NewText("Row " + string(rune('A'+i%26))),
...
func scrollRegionHorizontalChildren() []ScrollRegionChild {
	return []ScrollRegionChild{
		{
			Facet:     primitive.NewText(strings.Repeat("Wide content segment ", 8)),
