...
func (a *app) NewWindow(opts platform.WindowOptions) (platform.Window, error) {
	...
	values := []C.uint32_t{0, eventMask, C.uint32_t(colormap)}
...
func (s *shmSurface) VulkanInstanceExtensions() []string {
	return []string{"VK_KHR_surface", "VK_KHR_xcb_surface"}
}

func (s *shmSurface) CreateVulkanSurface(instance uintptr) (uintptr, error) {
	if s == nil || s.conn == nil || s.win == 0 {
		return 0, errors.New("linux platform: Vulkan surface unavailable")
	}
	return rendervulkan.CreateXcbSurface(instance, uintptr(unsafe.Pointer(s.conn)), uint32(s.win), uint32(s.width), uint32(s.height))
}
		...
		if etype == C.XCB_SELECTION_NOTIFY {
			...
			if C.lurpic_selection_notify_requestor(sn) == C.xcb_window_t(owner) &&
				...
				reply := C.xcb_get_property_reply(
					conn,
					C.xcb_get_property(conn, 0, owner, prop, atomUTF8String, 0, 1<<20),
					nil,
				)
				if reply == nil {
...
func (s *shmSurface) Resize(width, height int) { _ = s.resize(width, height) }
